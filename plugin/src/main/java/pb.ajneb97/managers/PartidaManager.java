...
public class PartidaManager {
        ...
        if (key.equalsIgnoreCase("3_lives")) {
            ...
            for (Entity e : entidades) {
                if (e != null && e.getType().equals(EntityType.PLAYER)) {
                    ...
                    JugadorPaintball victimPlayer = partida.getJugador(player.getName());
    ...
    public static void muereJugador(Partida partida, JugadorPaintball jugadorAtacante, final JugadorPaintball victimPlayer, PaintballBattle plugin, boolean lightning, boolean nuke) {
        ...
        Equipo targetTeam = partida.getEquipoJugador(victimPlayer.getJugador().getName());
        Equipo equipoAtacante = partida.getEquipoJugador(jugadorAtacante.getJugador().getName());
        if (targetTeam.equals(equipoAtacante)) {
            return;
        }

        if (lightning) {
            victimPlayer.getJugador().getWorld().strikeLightningEffect(victimPlayer.getJugador().getLocation());
        }
        FileConfiguration messages = plugin.getMessages();
        FileConfiguration config = plugin.getConfig();
        victimPlayer.aumentarMuertes();
        victimPlayer.setDeathLocation(victimPlayer.getJugador().getLocation().clone());
        victimPlayer.getJugador().sendMessage(ChatColor.translateAlternateColorCodes('&', messages.getString("killedBy").replace("%player%", jugadorAtacante.getJugador().getName())));
        String[] separados = config.getString("killedBySound").split(";");
        try {
            Sound sound = Sound.valueOf(separados[0]);
            victimPlayer.getJugador().playSound(victimPlayer.getJugador().getLocation(), sound, Float.valueOf(separados[1]), Float.valueOf(separados[2]));
        } catch (Exception ex) {
            Bukkit.getConsoleSender().sendMessage(ChatColor.translateAlternateColorCodes('&', PaintballBattle.prefix + "&7Sound Name: &c" + separados[0] + " &7is not valid."));
        }
        victimPlayer.setAsesinadoRecientemente(true);
        victimPlayer.setLastKilledBy(jugadorAtacante.getJugador().getName());
        targetTeam.disminuirVidas(1);

        Equipo equipo = partida.getEquipoJugador(victimPlayer.getJugador().getName());
        if (victimPlayer.getSelectedHat().equals("explosive_hat")) {
            Random r = new Random();
            int num = r.nextInt(100);
            if (num >= 80) {
                if (Bukkit.getVersion().contains("1.8")) {
                    victimPlayer.getJugador().getWorld().playEffect(victimPlayer.getJugador().getLocation(), Effect.valueOf("EXPLOSION_LARGE"), 2);
                } else {
                    victimPlayer.getJugador().getWorld().spawnParticle(Particle.EXPLOSION_LARGE, victimPlayer.getJugador().getLocation(), 2);
                }
                separados = config.getString("explosiveHatSound").split(";");
                try {
                    Sound sound = Sound.valueOf(separados[0]);
                    victimPlayer.getJugador().getWorld().playSound(victimPlayer.getJugador().getLocation(), sound, Float.valueOf(separados[1]), Float.valueOf(separados[2]));
                } catch (Exception ex) {
                    Bukkit.getConsoleSender().sendMessage(ChatColor.translateAlternateColorCodes('&', PaintballBattle.prefix + "&7Sound Name: &c" + separados[0] + " &7is not valid."));
                }
                Collection<Entity> entidades = victimPlayer.getJugador().getWorld().getNearbyEntities(victimPlayer.getJugador().getLocation(), 5, 5, 5);
                for (Entity e : entidades) {
                    if (e != null && e.getType().equals(EntityType.PLAYER)) {
                        Player player = (Player) e;
                        JugadorPaintball secondVictimPlayer = partida.getJugador(player.getName());
        ...
        victimPlayer.getJugador().teleport(equipo.getSpawn());
        if (Bukkit.getVersion().contains("1.13") || Bukkit.getVersion().contains("1.14") || Bukkit.getVersion().contains("1.15")
                || Bukkit.getVersion().contains("1.16") || Bukkit.getVersion().contains("1.17") || Bukkit.getVersion().contains("1.18")
                || Bukkit.getVersion().contains("1.19") || Bukkit.getVersion().contains("1.20")) {
            if (victimPlayer.getSelectedHat().equals("chicken_hat")) {
                victimPlayer.getJugador().getInventory().removeItem(new ItemStack(Material.EGG));
            } else {
                victimPlayer.getJugador().getInventory().removeItem(new ItemStack(Material.SNOWBALL));
            }
        } else {
            if (victimPlayer.getSelectedHat().equals("chicken_hat")) {
                victimPlayer.getJugador().getInventory().removeItem(new ItemStack(Material.EGG));
            } else {
                victimPlayer.getJugador().getInventory().removeItem(new ItemStack(Material.valueOf("SNOW_BALL")));
            }
        }
        ...
        if (lastKilledBy != null && lastKilledBy.equals(victimPlayer.getJugador().getName())) {
        ...
        if (nuke) {
            ...
            for (JugadorPaintball j : partida.getJugadores()) {
                if (!j.getJugador().getName().equals(jugadorAtacante.getJugador().getName())) {
                    j.getJugador().sendMessage(ChatColor.translateAlternateColorCodes('&', messages.getString("nukeKillMessage").replace("%team_player1%", targetTeamName)
                            .replace("%player1%", victimPlayer.getJugador().getName()).replace("%team_player2%", equipoAtacanteName)
                            .replace("%player2%", jugadorAtacante.getJugador().getName())));
                }
            }
        }
        jugadorAtacante.getJugador().sendMessage(ChatColor.translateAlternateColorCodes('&', messages.getString("kill").replace("%player%", victimPlayer.getJugador().getName())));
        if (!nuke) {
