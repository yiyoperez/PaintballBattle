...
public class CooldownManager {
    ...
    public void cooldownComenzarJuego(Partida partida, int cooldown) {
        ...
        for (int i = 0; i < jugadores.size(); i++) {
            jugadores.get(i).getPlayer().sendMessage(ChatColor.translateAlternateColorCodes('&', messages.getString("arenaStartingMessage").replace("%time%", tiempo + "")));
        }
    ...
    protected boolean ejecutarComenzarJuego(YamlDocument messages, YamlDocument config, String prefix) {
        if (partida != null && partida.getEstado().equals(GameState.STARTING)) {
    ...
    public void cooldownFaseFinalizacion(Partida partida, int cooldown, final Team ganador) {
        this.partida = partida;
        this.tiempo = cooldown;
        partida.setTiempo(tiempo);
        BukkitScheduler scheduler = Bukkit.getServer().getScheduler();
    ...
    protected boolean ejecutarComenzarFaseFinalizacion(Team ganador) {
        if (partida != null && partida.getEstado().equals(GameState.ENDING)) {
            partida.disminuirTiempo();
            if (tiempo == 0) {
