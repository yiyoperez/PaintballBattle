...
public class MySQL {
    ...
    public static boolean isEnabled(YamlDocument config) {
    ...
    public static ArrayList<JugadorDatos> getPlayerDataMonthly(PaintballBattle plugin) {
        ...
        int year = calendar.get(Calendar.YEAR);
        try {
            PreparedStatement statement = plugin.getConexionDatabase().getConnection().prepareStatement("SELECT * FROM " + plugin.getConexionDatabase().getTablePlayers() + " WHERE (Year=" + year + " AND Month=" + mes + " AND Global_Data=0)");
            ResultSet resultado = statement.executeQuery();
            while (resultado.next()) {
    ...
    public static ArrayList<JugadorDatos> getPlayerDataWeekly(PaintballBattle plugin) {
        ...
        try {
            PreparedStatement statement = plugin.getConexionDatabase().getConnection().prepareStatement("SELECT * FROM " + plugin.getConexionDatabase().getTablePlayers() + " WHERE (Year=" + year + " AND Month=" + mes + " AND Week=" + semana + " AND Global_Data=0)");
            ResultSet resultado = statement.executeQuery();
            while (resultado.next()) {
                String name = resultado.getString("Name");
                if (!yaContieneJugador(players, name)) {
    ...
    public static int[] getStatsTotalesWeekly(PaintballBattle plugin, String name, int mes, int year, int semana) {
        int[] cantidades = {0, 0, 0, 0}; //Wins,Loses,Ties,Kills
        try {
            PreparedStatement statement = plugin.getConexionDatabase().getConnection().prepareStatement("SELECT * FROM " + plugin.getConexionDatabase().getTablePlayers() + " WHERE (Name=? AND Year=" + year + " AND Month=" + mes + " AND Week=" + semana + " AND Global_Data=0)");
            statement.setString(1, name);
            ResultSet resultado = statement.executeQuery();
    ...
    public static int[] getStatsTotalesMonthly(PaintballBattle plugin, String name, int mes, int year) {
        int[] cantidades = {0, 0, 0, 0}; //Wins,Loses,Ties,Kills
        try {
            PreparedStatement statement = plugin.getConexionDatabase().getConnection().prepareStatement("SELECT * FROM " + plugin.getConexionDatabase().getTablePlayers() + " WHERE (Name=? AND Year=" + year + " AND Month=" + mes + " AND Global_Data=0)");
            statement.setString(1, name);
            ResultSet resultado = statement.executeQuery();
